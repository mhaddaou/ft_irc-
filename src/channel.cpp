...
/*   Updated: 2023/01/25 19:02:27 by mhaddaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int createNewChannel(Server *server, std::vector<std::string> buffer, int fd){
    ...
    if (checkIfThereIsPass(buffer) == EXIT_SUCCESS){
        ...
        server->map_clients[fd].setChannel(buffer[1]);
    }
    else{
        Ch._name = buffer[1];
        Ch._pass = false;
    ...
    rpl = ":" +server->map_clients[fd].getNickName() + " JOIN " + buffer[1] + "\r\n"
...
void joinToExistingChannel(Server *server, std::vector<std::string> buffer, int fd){
    std::string rpl = ":" + server->map_clients[fd].getNickName() + "!" + server->map_clients[fd].getName() + "@localhost JOIN " + buffer[1] + "\r\n"
